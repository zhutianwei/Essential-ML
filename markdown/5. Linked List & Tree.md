...
## 445. Add Two Numbers II (Math, Node-Iteration, Pointers) (Microsoft 7, Amazon 7)
O(M+N)  
O(N)
...
## 24. Swap Node in Pair（DFS-Recursion, Iteration-Pointers) (Microsoft 5->7, Facebook 3, Amazon 4)
...
class Solution:
    def boundaryOfBinaryTree(self, root: TreeNode) -> List[int]:
        ...
        def leftdfs(node, leftbd, rightbd):
            if not node:
                return
            ...
            leftdfs(node.left, leftbd and node.left, rightbd)
            leftdfs(node.right, leftbd and not node.left, rightbd)
                
        def rightdfs(node, leftbd, rightbd):
            if not node:
                return
            ...
            rightdfs(node.left, leftbd, rightbd and not node.right)
            rightdfs(node.right, leftbd, rightbd and node.right)
        ...
        leftdfs(root.left, True, False)
        rightdfs(root.right, False, True)
...
## 863. All Nodes Distance K in Binary Tree (Amazon 9, Facebook 8, Microsoft 5)
...
## 133. Clone Graph (N-ary Tree. DFS-Recursion) (Facebook 18, Amazon 11, Microsoft Onsite)  
