...
## 332. Reconstruct Itinerary (Amazon 8, Google 4, Facebook 4)
...
## 384. Shuffle an Array (List. Pointer) (Amazon Onsite 7, Microsoft 3)
...
## 981. Time Based Key-Value Store (Dict) (Google 10, Amazon Onsite 7-1)
...
#                     dfs(word[1:], nxt)
    ...
    def autocomplete(self, cur, word):
        for c in cur:
            if c == '#':
                self.res.append(word)

            else:
                self.autocomplete(cur[c], word + c)
```
